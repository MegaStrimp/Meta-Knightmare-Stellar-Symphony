///@description MKSS - Enemy - Ghoul Knight - AI - Walk and Attack - Parry - Step

function scr_MKSS_Enemy_GhoulKnight_AI_WalkAndAttack_Parry_Step()
{
	#region Setup
	if (enemyState_Setup)
	{
		...
		if (canParry)
		{
			with (obj_Attack)
			{
				if ((!isEnemy) and (isMelee) and (distance_to_object(other) <= 8))
				{
					...
					scr_Enemy_ChangeState_Step(other,other.GhoulKnight_Attack_Counter);
