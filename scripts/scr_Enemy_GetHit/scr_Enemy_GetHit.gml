...
function scr_Enemy_GetHit(targetEnemy,dealtDamage,dealtKnockback,knockbackAngle,attackOwner = -1)
{
	with (targetEnemy)
	{
		...
		scr_Debug_WriteLog(string(object_get_name(object_index)) + " " + string_copy(string(id),14,6) + " Received " + string(dmgFinal) + " Damage And " + string(finalKnockbackForce) + " Knockback At " + string(knockbackAngleFinal) + " Degrees From " + string(object_get_name(attackOwner.object_index)));
	}
}